...
<feed xmlns="http://www.w3.org/2005/Atom">
    ...
    <entry>
        ...
        {% else %}
        {% for category in post.categories.toArray() %}
        <category term="{{ category.name }}" scheme="{{ (url + post.lang + '/' + category.path) | uriencode }}"/>
        {% endfor %}
        {% for tag in post.tags.toArray() %}
        <category term="{{ tag.name }}" scheme="{{ (url + post.lang + '/' + tag.path) | uriencode }}"/>
        {% endfor %}
        {% endif %}
    </entry>
    {% endfor %}
