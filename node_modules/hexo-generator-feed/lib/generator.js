'use strict';

var nunjucks = require('nunjucks');
var env = new nunjucks.Environment();
var pathFn = require('path');
var fs = require('fs');

env.addFilter('uriencode', function (str) {
    return encodeURI(str);
});

var atomTmplSrc = pathFn.join(__dirname, '../atom.xml');
var atomTmpl = nunjucks.compile(fs.readFileSync(atomTmplSrc, 'utf8'), env);
var rss2TmplSrc = pathFn.join(__dirname, '../rss2.xml');
var rss2Tmpl = nunjucks.compile(fs.readFileSync(rss2TmplSrc, 'utf8'), env);

module.exports = function (locals) {
    var config = this.config;
    var feedConfig = config.feed;
    var template = feedConfig.type === 'rss2' ? rss2Tmpl : atomTmpl;

    var posts = locals.posts.sort('-date');

    var url = config.url;
    if (url[url.length - 1] !== '/') url += '/';

    var languages = config.language;
    if (!Array.isArray(languages)) {
        languages = [languages];
    }

    var isMulti = languages.length > 1;
    return languages.reduce(function (result, lang, index, array) {
        var langPath = isMulti ? lang.toString() + '/' : '/';
        var filteredPosts = isMulti ? posts.filter(function (post) {
            return lang === post.lang || post.language;
        }) : posts;
        if (feedConfig.limit) {
            filteredPosts = filteredPosts.limit(feedConfig.limit);
        }
        var xml = template.render({
            config: config,
            url: url,
            posts: filteredPosts,
            feed_url: config.root + langPath + feedConfig.path
        })
        var data = {
            path: langPath + feedConfig.path,
            data: xml
        }

        if (isMulti && lang === languages[0]) {
            var defaultXml = template.render({
                config: config,
                url: url,
                posts: filteredPosts,
                feed_url: config.root + "/" + feedConfig.path
            });
            var defaultData = {
                path: "/" + feedConfig.path,
                data: defaultXml
            }
            return result.concat(defaultData)
        } else {
            return result.concat(data);
        }
    }, []);
};